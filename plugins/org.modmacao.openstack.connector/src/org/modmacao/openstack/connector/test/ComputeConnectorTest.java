...
public class ComputeConnectorTest {
	...
	public void testOcciCreateAndDelete() {
		...
		OSClientV2 os = OpenStackHelper.getOSClient();
		
		// wait until server is active
		while (os.compute().servers().get(cut.getRuntimeID()).getStatus() != Status.ACTIVE) {
			try {
				Thread.sleep(2000);
		...
		System.out.println("Server started, now waiting for 20 seconds before deleting it.");
		System.out.println(os.compute().servers().get(cut.getRuntimeID()).getAccessIPv4());
