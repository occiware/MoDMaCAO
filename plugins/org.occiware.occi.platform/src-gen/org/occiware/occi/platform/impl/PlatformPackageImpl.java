 ...
 */
package org.occiware.occi.platform.impl;
...
import org.occiware.occi.platform.App_tpl;
import org.occiware.occi.platform.Application;
import org.occiware.occi.platform.Component;
import org.occiware.occi.platform.Componentlink;
import org.occiware.occi.platform.Database;
import org.occiware.occi.platform.Databaselink;
import org.occiware.occi.platform.PlatformFactory;
import org.occiware.occi.platform.PlatformPackage;
import org.occiware.occi.platform.Res_tpl;
import org.occiware.occi.platform.Status;

import org.occiware.occi.platform.util.PlatformValidator;
...
public class PlatformPackageImpl extends EPackageImpl implements PlatformPackage {
	 ...
	 * @see org.occiware.occi.platform.PlatformPackage#eNS_URI
	 * @see #init()
	 * @generated
	 */
	...
	public EOperation getComponentlink__TargetConstraint__DiagnosticChain_Map() {
		return componentlinkEClass.getEOperations().get(0);
	}
	...
	public void createPackageContents() {
		...
		createEOperation(applicationEClass, APPLICATION___CONFIGURE);
		createEOperation(applicationEClass, APPLICATION___DEPLOY);
		createEOperation(applicationEClass, APPLICATION___UNDEPLOY);
		...
		createEOperation(componentEClass, COMPONENT___CONFIGURE);
		createEOperation(componentEClass, COMPONENT___DEPLOY);
		createEOperation(componentEClass, COMPONENT___UNDEPLOY);

		componentlinkEClass = createEClass(COMPONENTLINK);
		createEOperation(componentlinkEClass, COMPONENTLINK___TARGET_CONSTRAINT__DIAGNOSTICCHAIN_MAP);
	...
	public void initializePackageContents() {
		...
		initEOperation(getApplication__Configure(), null, "configure", 0, 1, IS_UNIQUE, IS_ORDERED);

		initEOperation(getApplication__Deploy(), null, "deploy", 0, 1, IS_UNIQUE, IS_ORDERED);

		initEOperation(getApplication__Undeploy(), null, "undeploy", 0, 1, IS_UNIQUE, IS_ORDERED);
		...
		initEOperation(getComponent__Configure(), null, "configure", 0, 1, IS_UNIQUE, IS_ORDERED);

		initEOperation(getComponent__Deploy(), null, "deploy", 0, 1, IS_UNIQUE, IS_ORDERED);

		initEOperation(getComponent__Undeploy(), null, "undeploy", 0, 1, IS_UNIQUE, IS_ORDERED);

		initEClass(componentlinkEClass, Componentlink.class, "Componentlink", !IS_ABSTRACT, !IS_INTERFACE, IS_GENERATED_INSTANCE_CLASS);

		EOperation op = initEOperation(getComponentlink__TargetConstraint__DiagnosticChain_Map(), ecorePackage.getEBoolean(), "targetConstraint", 0, 1, IS_UNIQUE, IS_ORDERED);
		...
		op = initEOperation(getApp_tpl__AppliesConstraint__DiagnosticChain_Map(), ecorePackage.getEBoolean(), "appliesConstraint", 0, 1, IS_UNIQUE, IS_ORDERED);
		addEParameter(op, ecorePackage.getEDiagnosticChain(), "diagnostics", 0, 1, IS_UNIQUE, IS_ORDERED);
		...
		addEEnumLiteral(statusEEnum, Status.UNDEPLOYED);
		addEEnumLiteral(statusEEnum, Status.DEPLOYED);
